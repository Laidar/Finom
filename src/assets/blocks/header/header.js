document.addEventListener('DOMContentLoaded', function(){
    ...
    dropdown.forEach(val => val.onclick = () => {
        if (!val.classList.contains('dropdown')) {
            return
        }
        let open = val.classList.contains('dropdown_open');

        if (!open) {
            dropdown.forEach(item => {
                item.classList.remove("dropdown_open");
            })
            val.classList.add("dropdown_open");
        } else {
            dropdown.forEach(item => {
                item.classList.remove("dropdown_open");
            })
    ...
    languageCountry.forEach(item => {
        ...
        item.onclick = (val) => {
            ...
            if (inputVal.checked) {
                ...
                let languageBlock = document.querySelectorAll('.language__block')

                languageBlock.forEach(val => {
                    let languageFlag = val.querySelector('.language__flag')
    ...
    languageItem.forEach(item => {
        ...
        item.onclick = () => {
            ...
            languageBlock.forEach(val => {
                let languageText = val.querySelector('.language__text')
